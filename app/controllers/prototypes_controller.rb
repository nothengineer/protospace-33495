class PrototypesController < ApplicationController
  ...
  def show
    ...
    if current_user.id == @prototype.user.id || current_user.position == '代表取締役' ||  current_user.position == '代表取締役副社長' || (current_user.position == 'マネージャー1' && @prototype.user.name == '田中 利奈' ) || (current_user.position == 'マネージャー2' && @prototype.user.name == '堀田 惇貴' )
      @comment = Comment.new
